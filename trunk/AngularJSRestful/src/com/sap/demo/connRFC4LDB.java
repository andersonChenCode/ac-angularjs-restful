...
public class connRFC4LDB {
public static void main(String[] args) throws JCoException {
		
		SapSystem sapSystem = SapUtilities.getLocalDefTestSystem();
    	Object[] obj = SapUtilities.getJCoFunction(sapSystem,"YFIRFC_LDB_F1S_DEMO");
    	Connection connect = (Connection)obj[0];
    	JCoFunction function = (JCoFunction)obj[1];
    	
    	//function.getImportParameterList().setValue("BUKRS", "CN01");
    	//function.getImportParameterList().setValue("GJAHR", "2012");
    	
		connect.executeNTX(function);
		
		JCoTable T_SPFLI = function.getTableParameterList().getTable("T_SPFLI");
		JCoTable T_SFLIGHT = function.getTableParameterList().getTable("T_SFLIGHT");
		

	    for (int i = 0; i < T_SPFLI.getNumRows(); i++){ 
	    	 T_SPFLI.setRow(i); 
	    	 System.out.println(" COUNTRYFR >>  " + T_SPFLI.getString("COUNTRYFR") );
	 		 JCoFieldIterator iterator = T_SPFLI.getFieldIterator();
		     while(iterator.hasNextField()) {
		       System.out.println(iterator.nextField().getName() + " >> " + iterator.nextField().getValue());
	         }
		     System.out.println("---");
	    }
	    System.out.println("========================================================");
	    for (int i = 0; i < T_SFLIGHT.getNumRows(); i++){ 
	    	T_SFLIGHT.setRow(i); 
	    	 String CARRID = T_SFLIGHT.getString("CARRID");
	    	 String PRICE = T_SFLIGHT.getString("PRICE");
	    	 System.out.println(CARRID +"   " + PRICE );
	    }
